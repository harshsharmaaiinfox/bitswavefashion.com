...
class Utility {
	...
	public function register_notice() {
		...
		if ( isset( rtlc_is_valid()['domain_match'] ) && ! rtlc_is_valid()['domain_match'] ) {
			$message = sprintf(
				...
				__( 'Your license key doesn\'t match your current domain. Please activate the license again for your current domain.', 'metro' ),
				menu_page_url( 'rtlc', false ),
				__( 'Activate License', 'metro' ),
			);
		} else {
			$message = sprintf(
				'<b>%1$s <a href="%2$s" target="_blank">%3$s</a> %4$s <a href="%5$s">%6$s</a></b>',
				__( 'Please activate your theme using Envato', 'metro' ),
				esc_url( 'https://help.market.envato.com/hc/en-us/articles/202822600-Where-Is-My-Purchase-Code-' ),
				__( 'purchase code', 'metro' ),
				__( 'to get full theme functionality and customer support.', 'metro' ),
				menu_page_url( 'rtlc', false ),
				__( 'Activate License', 'metro' ),
			);
		}
	...
	public function script() {
		...
		if ( wp_script_is( 'jquery', 'done' ) && ! rtlc_is_valid()['success'] ) {
			...
			if ( isset( $_GET['page'] ) && 'metro-install-plugins' === $_GET['page'] ) {
				?>
				<script type="text/javascript">  
...
if ( ! function_exists( 'rtlc_is_valid' ) ) {
	...
	function rtlc_is_valid() {
		...
		} elseif ( isset( $get_option[ $metro . '_license' ] ) && isset( $get_option[ $metro . '_license' ]['key'] ) ) {
