            ...
            <div class="slider-text-content">
               ...
               <?php foreach ( $data['items'] as $item ) : ?>
				...
				?>
 				 <div class="slick-slide">			 		
			 		<?php if (  $item['title'] ): ?>
                    <h2 class="item-title"><?php echo wp_kses_post( $item['title'] );?></h2>
                    <?php endif; ?>	
                	...
                	<?php if ( $item['linktext'] ): ?>
	                    <div class="item-btn">
							<a <?php echo $attr;?> class="fas fa-long-arrow-alt-right"><?php echo esc_html( $item['linktext'] );?> <span class="fas fa-long-arrow-alt-right"></span></a>
						</div>
					<?php endif; ?>	                        	
                    </div>
                    <?php endforeach; ?>                    
        ...
        <div class="share-btn-wrap">
             <a href="#" class="item-btn"><?php echo wp_kses_post( $data['share_title'] );?> <span>+</span></a>
                <ul class="share-icon">                  
                    ...
                    $icon = '<span class="'.$share['share_icon']['value'].'" aria-hidden="true"></span>';
                     ?><li><a href="<?php echo esc_url( $share['share_url']['url'] );?>" title="<?php _e( 'Social', 'metro-core' ); ?>">   <?php echo  $icon; ?></a></li>                   
                    <?php endforeach; ?>
                </ul>
         </div>
     <?php } ?>
